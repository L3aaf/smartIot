...
void LED_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);						//将PC13引脚初始化为推挽输出
...
void LED1_OFF(void)
{
	GPIO_ResetBits(GPIOA, GPIO_Pin_4);		//设置PA1引脚为低电平
}
...
void LED1_Turn(void)
{
	...
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_4);					//则设置PC13引脚为高电平
	}
	else													//否则，即当前引脚输出高电平
	{
		GPIO_ResetBits(GPIOA, GPIO_Pin_4);					//则设置PC13引脚为低电平
	}
}
